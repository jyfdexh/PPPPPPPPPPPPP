...
class UiConfigTests(unittest.TestCase):
    def test_public_ui_config_exposes_proxy_controls(self) -> None:
        original_proxy = app.DEFAULT_PROXY
        app.DEFAULT_PROXY = "http://user-region-JP:pass@127.0.0.1:3010"
        try:
            config = app.build_ui_config("public")
        finally:
            app.DEFAULT_PROXY = original_proxy
        ...
        self.assertFalse(config["proxy_defaults"]["custom_proxy"])
        self.assertEqual(config["approve_defaults"]["attempt_count"], 10)
        self.assertEqual(config["approve_defaults"]["max_attempt_count"], 10)
